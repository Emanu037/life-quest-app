...
class _PersonagemPageState extends State<PersonagemPage> {
  ...
  Future<void> carregarNomeMascote() async {
    final prefs = await SharedPreferences.getInstance();

    setState(() {
      nomeMascote = prefs.getString('petName') ?? 'GATINHO';
    });
  }
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ListView(
                        ...
                        children: mostrarLoja
                            ? _buildLoja(vm)
                            : _buildGuardaRoupa(vm),
                      ),
                    ),
                    const SizedBox(height: 20),
                  ],
  ...
  List<Widget> _buildGuardaRoupa(AppViewModel vm) {
    if (vm.guardaRoupa.isEmpty) {
      return [
        const Center(
          child: Text("Você ainda não tem roupas. Vá à loja!"),
        )
      ];
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color:
                selecionado ? Colors.brown : Colors.brown.withOpacity(0.1),
            width: 2,
          ),
        ),
        child: Column(
      ...
      children: [
        SizedBox(
          ...
          child: FloatingActionButton(
            ...
            child: Icon(
              icone,
              color: ativo ? Colors.white : Colors.brown,
            ),
          ),
        ),
